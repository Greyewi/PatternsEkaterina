...
class AlarmDecorator {
  constructor(alarm, frequency) {
    this.alarm = alarm
    this.alarmCollection = this.alarm.alarmCollection
    this.timerId = this.alarm.timerId
    this.frequency = frequency
  }

  start() {
    const checkClock = (clock) => {
      if (clock.time === this.alarm.getCurrentFormattedTime()) {
        clock.callback()
      }
    }
    const setNewInterval = () => {
    ...
    if (!this.timerId) {
      this.timerId = setInterval(()=> setNewInterval(), this.frequency)
    }
...
newAlarmClock.addClock('11:15', () => console.log('Пора вставать!'),1);

const myFrequencyAlarm = new AlarmDecorator(newAlarmClock, 2000)
myFrequencyAlarm.start()